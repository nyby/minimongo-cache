...
describe('selector', function () {
  ...
  describe(compileDocumentSelector.name, function () {
    ...
    describe('field selector', function () {
      const fn = () => {}
      const values = [undefined, null, 'foo', 13579, true, ['foo'], { foo: 'bar' }, fn]
      ...
      values.forEach(value => {
        ...
        it(`compiles a selector for a ${type} field`, function () {
          ...
          ;[undefined, null, 1, '1', '', 'bar', true, false, {}, [], new Date(), () => {}, randomHex()]
            .forEach(_id => {
              if (_id == value) return
              expect(selector({ _id }),JSON.stringify( { value, _id })).to.deep.equal(false)
            })
        })
      })
      ...
      it('throws on inconsistent selectors', function () {
        expect(() => compileDocumentSelector({ _id: { $lt: 1, lt: 2 }}))
          .to.throw('Inconsistent selector: ')
      })
      it('throws on unrecognized operator', function () {
        expect(() => compileDocumentSelector({ _id: { $foo: [1, 2, 3]} }))
          .to.throw('Unrecognized operator: $foo')
      })
    })

    describe('value operators', function () {
      const falsyValues = [undefined, null, 1, '1', '', 'bar', true, false, {}, [], new Date(), () => {}, randomHex()]

      it('$in', function () {
        expect(() => compileDocumentSelector({ _id: { $in: 1 }}))
          .to.throw('Argument to $in must be array')

        const value1 = randomHex()
        const value2 = randomHex()
        const selector = compileDocumentSelector({ _id: { $in: [value1, value2]} })
      ...
      it('$all', function () {
        expect(() => compileDocumentSelector({ _id: { $all: 1 }}))
          .to.throw('Argument to $all must be array')

        const selector = compileDocumentSelector({ _id: { $all: ['foo', 'bar']}})
        expect(selector({ _id: ['foo', 'bar']})).to.equal(true)
        ...
        // nested
        const selector1 = compileDocumentSelector({ _id: { $all: [['foo'], ['bar']]}})
      ...
      it('$lt', function () {
        ...
        expect(selector({ _id: Number.MAX_SAFE_INTEGER * 2 })).to.deep.equal(false)
      ...
      it('$gt', function () {
        ...
        expect(selector({ _id: Number.MAX_SAFE_INTEGER * 2 })).to.deep.equal(true)
      ...
      it('$nin', function () {
        expect(() => compileDocumentSelector({ _id: { $nin: 1 }}))
          .to.throw('Argument to $nin must be array')

        const value1 = randomHex()
        const value2 = randomHex()
        const selector = compileDocumentSelector({ _id: { $nin: [value1, value2]} })
      ...
      it('$type', function () {
        const test = (info, $type, truthy, falsey) => {
          ...
          truthy.forEach(_id => expect(selector({ _id }), _id).to.equal(true))
          falsey.forEach(_id => expect(selector({ _id }), _id).to.equal(false))
        }
        ...
        test('number', 1, [1, -1, 0, 1.1, Infinity], [date, bin, '0', '1', {}, [], ()=>{},true, false, null, undefined])
        test('string', 2, ['foo', '', ' ', '1'], [date, bin, 0, /foo/, {}, [], ()=>{},true, false, null, undefined])
        test('boolean', 8, [true, false], [date, bin, 0, 1, '', ' ', 'wqe', {}, ()=>{}, [], null, undefined])
        test('array', 4, [[[]]], [date, bin, 0, 1, '', ' ', 'wqe', {}, null, undefined, ()=>{}])
        test('regex', 11, [/foo/], [date, bin, 0, 1, '', ' ', 'wqe', {}, [], null, undefined, ()=>{}])
        test('date', 9, [new Date()], [bin, Date, 0, 1, '', ' ', 'wqe', {}, [], null, undefined])
        test('binary', 5, [bin], [date, 0, 1, '', ' ', 'wqe', {}, [], null, undefined])
        test('object', 3, [{}, new Custom()], [date, 0, 1, '', ' ', 'wqe', [], null, undefined])
      })
      it('$regex', function () {
        expect(() => compileDocumentSelector({ _id: {
            $regex: /pattern/, $options: 'foo'
      ...
      it('$elemMatch', function () {
        // nested objects
        const selector = compileDocumentSelector({ _id: { $elemMatch: { foo: 'bar', bar: { $gt: 0 } } } })
      ...
      it('$not', function () {
        const selector = compileDocumentSelector({ _id: { $not: { foo: 'bar' }} })
    ...
    describe('logical operators', function () {
      it('throws on unrecognized operator', function () {
        expect(() => compileDocumentSelector({ $foo: 2 }))
          .to.throw('Unrecognized logical operator: $foo')
      })
      it('$and', function () {
        expect(() => compileDocumentSelector({ $and: 1 })).to.throw('$and/$or/$nor must be nonempty array')
        expect(() => compileDocumentSelector({ $and: [] })).to.throw('$and/$or/$nor must be nonempty array')
      ...
      it('$or', function () {
        expect(() => compileDocumentSelector({ $or: 1 })).to.throw('$and/$or/$nor must be nonempty array')
        expect(() => compileDocumentSelector({ $or: [] })).to.throw('$and/$or/$nor must be nonempty array')
      ...
      it('$nor', function () {
        expect(() => compileDocumentSelector({ $nor: 1 })).to.throw('$and/$or/$nor must be nonempty array')
        expect(() => compileDocumentSelector({ $nor: [] })).to.throw('$and/$or/$nor must be nonempty array')
