...
describe('ReadTransaction', function () {
  ...
  describe('canPushTransaction', function () {
    it('can only push SynchronousWriteTransacitons', function () {
      expect(transaction.canPushTransaction(new SynchronousWriteTransaction())).to.equal(true)
      ...
      ;[
        ...
        ReadOnlyTransaction,
        WriteTransaction
       ].forEach(T => expect(transaction.canPushTransaction(new T())).to.equal(false))
    })
  })
})
